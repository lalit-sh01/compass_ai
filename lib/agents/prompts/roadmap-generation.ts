import { GOLD_STANDARD_WEEK_1, QUALITY_MARKERS } from './gold-standard-examples'
import { formatResourceDatabase } from '../resources/ai-pm-resources'

export const ROADMAP_GENERATION_SYSTEM_PROMPT = `You are an expert learning roadmap architect. Your role is to create comprehensive, week-by-week learning roadmaps that help users achieve their goals.

## GOLD STANDARD EXAMPLE

Here is an example of EXCELLENT week that demonstrates the quality expected:

${JSON.stringify(GOLD_STANDARD_WEEK_1, null, 2)}
...
export function buildRoadmapGenerationPrompt(
  ...
  return `Generate a comprehensive learning roadmap with the following requirements:

## User Profile:
- **Goal**: ${assessment.goal.title}
- **Timeframe**: ${assessment.goal.timeframe} weeks
- **Current Skills**: ${assessment.currentSkills.technical.join(', ')}
- **Experience**: ${assessment.currentSkills.experience.years} years (${assessment.currentSkills.experience.roles.join(', ')})
- **Learning Style**: ${assessment.learningPreferences.style}
- **Hours per Week**: ${assessment.learningPreferences.hoursPerWeek} hours
- **Budget**: ${assessment.constraints.budget}
- **Preferred Resources**: ${assessment.learningPreferences.preferredResources.join(', ')}

## Skills to Focus On (Selected by User):
${skillsList}

## Gap Analysis Summary:
**Essential Skills**: ${gapAnalysis.targetRole.requiredSkills.essential.map((s: any) => s.skill).join(', ')}
**Recommended Skills**: ${gapAnalysis.targetRole.requiredSkills.recommended.map((s: any) => s.skill).join(', ')}

**Learning Path Phases**:
1. ${gapAnalysis.learningPath.phase1.title} (${gapAnalysis.learningPath.phase1.duration} weeks): ${gapAnalysis.learningPath.phase1.focus.join(', ')}
2. ${gapAnalysis.learningPath.phase2.title} (${gapAnalysis.learningPath.phase2.duration} weeks): ${gapAnalysis.learningPath.phase2.focus.join(', ')}
3. ${gapAnalysis.learningPath.phase3.title} (${gapAnalysis.learningPath.phase3.duration} weeks): ${gapAnalysis.learningPath.phase3.focus.join(', ')}

**Strengths to Leverage**: ${gapAnalysis.recommendations.strengths.join(', ')}
**Quick Wins**: ${gapAnalysis.recommendations.quickWins.join(', ')}

${formatResourceDatabase()}

## Requirements:
1. Create exactly ${assessment.goal.timeframe} weeks of content
2. Each week should have ${assessment.learningPreferences.hoursPerWeek} hours total
...
10. The roadmap must conform EXACTLY to this JSON structure:
{
  ...
  "timeCommitmentPerWeek": "string - e.g., '${assessment.learningPreferences.hoursPerWeek} hours/week'",
  "profile": {

...
import { Play, Plus, Upload, Settings, Clock, ChevronDown, Zap, Search, PanelLeftClose, PanelLeft, Archive, Share2, Trash2 } from 'lucide-react';
...
// Mock analytics data (this could be fetched from backend later)
const mockAnalytics = {
    weeklyActivity: [
        { day: 'Mon', hours: 2.5 },
        { day: 'Tue', hours: 3.2 },
...
export default function DashboardContent({ roadmaps: initialRoadmaps, hasApiKey, stats }: DashboardContentProps) {
    ...
    const formatDate = (date: Date | string | undefined | null) => {
        ...
        // Safety check for invalid dates or non-date objects
                                            ...
                                            {mockAnalytics.weeklyActivity.map((day, i) => (
                                                <div key={day.day} className="flex-1 flex flex-col items-center gap-1">
                                                    <motion.div
                                                        ...
                                                        className="w-full bg-primary/80 rounded-sm min-h-[4px]"
                                                        title={`${day.hours}h`}
                                                    />
                            ...
                            <div className="pt-4 border-t border-border">
                                ...
                                <div className="space-y-2 text-[10px]">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <kbd className="px-1.5 py-0.5 rounded bg-bg-secondary border border-border text-primary font-mono text-[9px]">N</kbd>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-text-secondary">Search</span>
                                        <kbd className="px-1.5 py-0.5 rounded bg-bg-secondary border border-border text-primary font-mono text-[9px]">/</kbd>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="px-8 py-4">
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4 flex-1">
                                ...
                                <div className="flex-1 max-w-md relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-2 bg-surface border border-border rounded-lg text-sm text-text-primary placeholder:text-text-secondary focus:outline-none focus:ring-2 focus:ring-primary/20"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                <Link
                                    href="/onboarding"
                                    className={`inline-flex items-center gap-2 px-4 py-2 bg-primary text-on-primary rounded-lg hover:bg-primary/90 transition-colors font-medium text-sm shadow-sm ${!hasApiKey ? 'opacity-50 pointer-events-none' : ''}`}
                                >
                    ...
                    {filteredRoadmaps.length === 0 && (
                        <div className="text-center py-12">
                            ...
                            <p className="text-sm text-text-secondary">
                                {searchQuery || filterStatus !== 'all' ? 'Try adjusting your search or filters' : 'Get started by creating your first roadmap'}
                            </p>
                            {!searchQuery && filterStatus === 'all' && (
                                <Link
                                    href="/onboarding"
                                    className={`inline-flex items-center gap-2 px-6 py-3 mt-6 bg-primary text-on-primary rounded-lg hover:bg-primary/90 transition-colors font-medium shadow-sm ${!hasApiKey ? 'opacity-50 pointer-events-none' : ''}`}
                                >
                                                ...
                                                >
                                                    {roadmapsInSection.map((roadmap, index) => (
                                                        <motion.div
                                                            ...
                                                            className={`group relative ${isDeleting === roadmap.id ? 'opacity-50 pointer-events-none' : ''}`}
                                                        >
                                                            <Link href={`/viewer/${roadmap.id}`}>
                                                                <div className="relative overflow-hidden rounded-xl border border-border bg-gradient-to-br from-surface to-primary/5 p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 hover:border-primary/50 hover:to-primary/10">
                                                                    ...
                                                                    {/* Title */}
                                                                    <h3 className="text-base font-semibold text-text-primary font-primary mb-4 line-clamp-2 group-hover:text-primary transition-colors">
                                                                        {roadmap.title}
                                                                    </h3>

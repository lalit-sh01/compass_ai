...
export default function AssessmentWizard({ onComplete }: AssessmentWizardProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const data = await apiClient.post('/api/agents/assessment', { answers })
      onComplete(data)
    } catch (err) {
          ...
          <div className="space-y-2">
            {currentQuestion.options.map((option) => (
              <button
                ...
                className={`w-full px-4 py-3 text-left border-2 rounded-lg transition-all ${answers[currentQuestion.id] === option.value

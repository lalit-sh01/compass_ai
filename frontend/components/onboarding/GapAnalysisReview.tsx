...
export default function GapAnalysisReview({ assessment, onComplete }: GapAnalysisReviewProps) {
  ...
  const runGapAnalysis = async () => {
    ...
    try {
      const data = await apiClient.post('/api/agents/gap-analysis', { assessment })

      setGapAnalysis(data)

      // Auto-select all essential and recommended skills
      const autoSelected = new Set([
        ...data.targetRole.requiredSkills.essential.map((s: SkillGap) => s.skill),
        ...data.targetRole.requiredSkills.recommended.map((s: SkillGap) => s.skill),
      ])
    ...
    return (
      <div className="bg-surface rounded-lg shadow-sm p-12 text-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <h3 className="text-lg font-semibold text-text-primary">Analyzing Your Skills...</h3>
    ...
    return (
      <div className="bg-surface rounded-lg shadow-sm p-8">
        ...
        <button
          ...
          className="mt-4 px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover"
        >
          Try Again
        </button>
      </div>
  ...
  return (
    <div className="bg-surface rounded-lg shadow-sm p-8">
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-text-primary mb-2">Your Personalized Gap Analysis</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
          <h3 className="font-semibold text-text-primary mb-2">💪 Your Strengths</h3>
      ...
      {/* Skill Selection */}
      <div className="mb-8">
        <h3 className="text-lg font-semibold text-text-primary mb-4">Select Skills to Include in Your Roadmap</h3>
      ...
      <div className="space-y-2">
        {skills.map((skill) => (
          <button
            ...
            className={`w-full p-4 text-left border-2 rounded-lg transition-all ${selectedSkills.has(skill.skill)

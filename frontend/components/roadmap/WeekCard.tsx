...
export default function WeekCard({ week, compact = false }: WeekCardProps) {
  ...
  return (
    <div className="bg-surface rounded-md shadow-sm border border-border overflow-hidden hover:shadow-md transition-shadow">
      <div className={compact ? 'p-[var(--space-4)]' : 'p-[var(--space-6)]'}>
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            ...
            <h4 className={`font-bold text-text-primary mb-1 ${compact ? 'text-base' : 'text-lg'}`}>{week.title}</h4>
            <p className={`text-text-secondary italic ${compact ? 'text-xs' : 'text-sm'}`}>{week.theme}</p>
          </div>
        </div>
          ...
          <>
            ...
            {week.buildSection && (
              <div className="mb-3 p-3 bg-primary/5 border border-primary/20 rounded-md">
                ...
                <div className="text-sm font-medium text-text-primary">{week.buildSection.projectTitle}</div>
              </div>
            )}
          </>
        )}

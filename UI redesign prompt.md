...
Analyze: Read the current code for the target view. Identify usability gaps, visual weaknesses, and missed opportunities for AI integration.
Ideate: Propose 3 distinct design concepts. For each concept, provide:
...
Create Route: Create a new route at /experiments/[concept-name].
Mock Data: Use static JSON data (do not connect to real backend yet) to simulate ideal states.
Build Components: Create new, isolated components in components/experiments/[concept-name]/.
...
Iterate: specific tweaks (e.g., "Move sidebar to right", "Make cards glassmorphic").
Micro-Interactions: Add polish like scroll-based focus, sticky headers, or skeleton loading states.
Responsiveness: Ensure it breaks down gracefully on smaller screens.
...
Migrate Components: Move components from experiments/ to components/viewer/[feature]/. Rename generic names to specific ones.
Connect Data: Replace mock data with real props and hooks from the main application context.
Replace Page: Overwrite the original 
page.tsx
 with the new layout.
Cleanup: Remove the experimental route and unused legacy components.
Output: The main application running the new design.

Instruction: Start by asking me for the View Name or File Path you want me to redesign.
...
export default function ImportRoadmap() {
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Failed to import roadmap')
      }

      setSuccess(true)

      // Redirect to the imported roadmap after a short delay
      setTimeout(() => {
        window.location.href = `/viewer?roadmapId=${result.roadmapId}`
      }, 1500)
    } catch (err) {

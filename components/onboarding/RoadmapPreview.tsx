...
export default function RoadmapPreview({
  ...
  const generateRoadmap = async () => {
    ...
    try {
      const response = await fetch('/api/agents/generate-roadmap', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ assessment, gapAnalysis, selectedSkills }),
      })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      window.location.href = `/viewer?roadmapId=${data.roadmap.id}`
    } catch (err) {
      setError((err as Error).message)
    } finally {
      setIsSaving(false)
